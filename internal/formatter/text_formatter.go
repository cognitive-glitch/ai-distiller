...
func (f *TextFormatter) formatDirect(w io.Writer, file *ir.DistilledFile) error {
	// Write file header
...
func (f *TextFormatter) FormatMultiple(w io.Writer, files []*ir.DistilledFile) error {
	// Generate dynamic distillation instructions based on processing options
	instructions := f.generateDistillationInstructions()
	if instructions != "" {
		fmt.Fprintln(w, instructions)
		fmt.Fprintln(w) // Add blank line after instructions
	}
