...
fn test_mixed_language_directory() {
    ...
    match result {
        Ok(directory) => {
            ...
            assert!(file_count >= 3, 
                   "Expected at least 3 files (py, ts, go), got {}", file_count);
            ...
            println!("✅ Mixed language processing: {} files processed", file_count);
        }
        Err(e) => {
...
fn test_empty_directory_handling() {
    ...
    match result {
        Ok(directory) => {
            assert_eq!(directory.children.len(), 0, 
...
fn test_parallel_processing_consistency() {
    ...
    assert_eq!(result2.children.len(), result3.children.len(),
              "Parallel processing should be deterministic");
    
    // File order should be preserved (sorted by discovery order)
    for (i, (node1, node2)) in result1.children.iter().zip(result2.children.iter()).enumerate() {
        if let (Node::File(f1), Node::File(f2)) = (node1, node2) {
            assert_eq!(f1.path, f2.path, 
                      "File order should be consistent at index {}", i);
        }
    }
...
fn test_recursive_vs_non_recursive() {
    ...
    if let (Ok(dir_recursive), Ok(dir_non_recursive)) = (result_recursive, result_non_recursive) {
        ...
        assert!(dir_recursive.children.len() >= dir_non_recursive.children.len(),
               "Recursive should find at least as many files as non-recursive");
        
        println!("✅ Recursive: {} files, Non-recursive: {} files", 
